...
export default class Fuse {
  constructor(web3Provider) {
    ...
    this.FUSE_FEE_DISTRIBUTOR_CONTRACT_ADDRESS =
      "0x4084A1CEb13516eEaa03a047679cE64DF5c7476A"; // TODO: Set correct mainnet address after deployment

    this.COMPTROLLER_IMPLEMENTATION_CONTRACT_ADDRESS =
      "0xE14E15c2a43232791bd35B326BC5446CC2Ad9CD7"; // TODO: Set correct mainnet address after deployment
    this.CERC20_DELEGATE_CONTRACT_ADDRESS =
      "0x99dF4ba7A2404e2cDFDa8c6427e03E13EA23D19f"; // TODO: Set correct mainnet address after deployment
    this.CETHER_DELEGATE_CONTRACT_ADDRESS =
      "0x026838c33C05caCe3495bfE51261f7D942fddf8A"; // TODO: Set correct mainnet address after deployment

    this.OPEN_ORACLE_PRICE_DATA_CONTRACT_ADDRESS =
      "0xc629c26dced4277419cde234012f8160a0278a79";
    this.COINBASE_PRO_REPORTER_ADDRESS =
      "0xfCEAdAFab14d46e20144F48824d0C09B1a03F2BC";

    this.PUBLIC_PREFERRED_PRICE_ORACLE_CONTRACT_ADDRESS =
      "0x68aa7c6be5eDbb8434dCE624b8583FE4c25Cac8B"; // TODO: Set correct mainnet address after deployment
    this.PUBLIC_CHAINLINK_PRICE_ORACLE_CONTRACT_ADDRESS =
      "0x5fdbfc67a415a811dd8e5de570ddf65207ceb94f"; // TODO: Set correct mainnet address after deployment
    this.PUBLIC_UNISWAP_VIEW_CONTRACT_ADDRESS =
      "0x68ae90e1da41761aad06a7f188584ca806012f5e"; // TODO: Set correct mainnet address after deployment

    this.DAI_POT = "0x197e90f9fad81970ba7976f33cbd77088e5d7cf7";
    this.DAI_JUG = "0x19c0976f590d67707e62397c87829d896dc0f1f1";

    this.UNISWAP_V2_FACTORY_ADDRESS =
      "0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f";
    this.UNISWAP_V2_PAIR_INIT_CODE_HASH =
      "0x96e8ac4277198ff8b6f785478aa9a39f403cb768dd02cbee326c3e7da348845f";
    this.WETH_ADDRESS = "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2";

    this.PRICE_ORACLE_RUNTIME_BYTECODE_HASHES = {
    ...
    this.deployCEther = async function (
      ...
      if (collateralFactor)
        await comptroller.methods
          ...
          .send(options);
      if (reserveFactor)
        await cEtherDelegator.methods
          ._setReserveFactor(reserveFactor)
          .send(options);
      if (adminFee)
        await cEtherDelegator.methods._setAdminFee(adminFee).send(options);
    ...
    this.deployCErc20 = async function (
      ...
      if (!bypassPriceFeedCheck) {
        ...
        if (
          ...
          try {
            ...
            var underlyingSymbol = await underlyingToken.methods
              .symbol()
              .call();
            ...
            if (
              conf.underlying == "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2"
            ) {
              // WETH
              await uniswapOrUniswapAnchoredView.methods
                .add([
      ...
      if (collateralFactor)
        await comptroller.methods
          ...
          .send(options);
      if (reserveFactor)
        await cErc20Delegator.methods
          ._setReserveFactor(reserveFactor)
          .send(options);
      if (adminFee)
        await cErc20Delegator.methods._setAdminFee(adminFee).send(options);
