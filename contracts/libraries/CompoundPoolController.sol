...
library CompoundPoolController {
    ...
    /**
        ...
        @param _comptroller The address of Compound's Comptroller
    */
    function deposit(
        address _underlying,
        uint256 _amount,
    ...
    */
    ...
    /**
        @dev Get the maximum borrow amount for a specific currency
        @param _underlyingCollateral The address of the underlying token that serves as collateral
        @param _amount The amount of collateral that the user has deposited
        @param _underlyingBorrow The address of the token to be borrowed
        @param _comptroller The address of Compound's comptroller
        @param _priceFeed The address of Compound's price feed
    */
    function getMaxUnderlyingBorrowAmount(
        ...
        address collateralCErc20Contract = getCErc20Contract(_underlyingCollateral);
        CErc20 collateral = CErc20(collateralCErc20Contract);
        CErc20 borrowing = CErc20(getCErc20Contract(_underlyingBorrow));
        Comptroller comptroller = Comptroller(_comptroller);
        PriceFeed priceFeed = PriceFeed(_priceFeed);

        // Get the collateral factor of the collateralized asset
        (, uint256 collateralFactorMantissa) =
            comptroller.markets(collateralCErc20Contract);

        // Calculate the USD Balance of the user
        uint256 underlyingBalanceMantissa = _amount
            .mul(collateral.exchangeRateCurrent())
            .div(1e18);

        uint256 usdBalanceMantissa = underlyingBalanceMantissa
            .mul(priceFeed.getUnderlyingPrice(collateralCErc20Contract))
            .div(1e18);

        // Calculate the total USD borrow amount
        uint256 usdBorrowAmountMantissa = usdBalanceMantissa
            .mul(collateralFactorMantissa)
            .div(1e18);

        // Return the total borrow supply for the asset
        return usdBorrowAmountMantissa
            .mul(priceFeed.getUnderlyingPrice(getCErc20Contract(_underlyingBorrow)))
            .div(1e18);
    }
